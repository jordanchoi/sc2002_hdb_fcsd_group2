...
public class BTOProjectsView implements UserView {
    ...
    public void INSERTDUMMYVALUE(HDBBTOExerciseController exerciseController, BTOProjsController projsController) {
        if (exerciseController.viewAllExercises().isEmpty()) {
            System.out.println("No exercises exist. Please create a BTO exercise first.");
            return;
        }

        // Get first exercise to attach the dummy projects
        BTOExercise selectedExercise = exerciseController.viewAllExercises().get(0);

        // Dummy Manager
        HDBManager manager = (HDBManager) SessionStateManager.getInstance().getLoggedInUser();

        // Dummy project 1
        BTOProj p1 = projsController.CreateProj(
                1001,
                "Project Alpha",
        ...
        p1.setProjNbh(Neighbourhoods.Tampines);
        p1.addFlatType(FlatTypes.TWO_ROOM, 50);
        p1.addFlatType(FlatTypes.THREE_ROOM, 30);
        p1.setManagerIC(manager);
        p1.setOfficerSlotLimit(10);
        selectedExercise.addProject(p1);

        // Dummy project 2
        BTOProj p2 = projsController.CreateProj(
                1002,
                "Project Beta",
        ...
        p2.setProjNbh(Neighbourhoods.Yishun);
        p2.addFlatType(FlatTypes.TWO_ROOM, 20);
        p2.addFlatType(FlatTypes.THREE_ROOM, 40);
        p2.setManagerIC(manager);
        p2.setOfficerSlotLimit(10);
        selectedExercise.addProject(p2);

        System.out.println("2 dummy projects inserted into exercise: " + selectedExercise.getExerciseName());
    }
